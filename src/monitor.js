...
const{dataWash1} = require('./utils');

(init = async ()=> {
    let env = 'mainnet'
    let pairDir = './data/top_200_pair_list.json'
    let routesDir = './data/total_route5_1.json'
    let pathDir = './data/total_path5_1.json'
    ...
    ethersWSChainProvider.on("block",  async(blockNumber, err) =>{
        try{
            if (lock == false){
                ...
                let pairsInfo = dataWash1(result, pairList)
                console.log('Block ' + blockNumber.toString() + ': calculating best trade')
                let bestTrades = findRouteArb(paths[0], pairsInfo, paths[2], tokenIn)
                ...
                let newArr1 = JSON.stringify(bestTrades)
                fs.writeFileSync('./opportunity/best_trade.json', newArr1, 'utf8', (err) =>{
...
function findRouteArb(routes, pairsInfo, paths, tokenIn){
    ...
    for(let i = 0; i < routes.length; i ++){
        ...
        let eb = result[1]
        let newTrade = {}
        newTrade = {'routes': tempCurrentPairs, 'route': route, 'path': paths[i], 'ea': ea, 'eb': eb}//
        if (ea.gt(BN.from(0)) && eb.gt(BN.from(0)) && ea.lt(eb)){
            newTrade ['optimalAmount'] = getOptimalAmount(ea, eb)
            newTrade['outputAmount'] = getAmountOut(newTrade['optimalAmount'], ea, eb)
            // newTrade['outputAmount'] = getAmountOutByPath(weth, newTrade ['optimalAmount'], tempCurrentPairs)
            newTrade['profit'] = newTrade['outputAmount'].sub(newTrade['optimalAmount'])
            newTrade['p'] = newTrade['profit'].div(BN.from((10**tokenIn['decimals']).toString()))
        }
        else{
            continue
        }
